  ...
  return dashData && (
    <div className='m-5 text-stone-800'>
      ...
      <div className='mb-8'>
        ...
        <p className='text-stone-500 mt-2'>Welcome back! Here's an overview of your hospital management system</p>
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-10'>
        ...
        <div className='bg-gradient-to-br from-primary/10 to-teal-500/10 border border-primary/20 p-6 rounded-2xl hover:border-primary/40 hover:shadow-xl hover:shadow-primary/20 transition-all duration-300 group'>
          <div className='flex items-center gap-4'>
            ...
            <div>
              <p className='text-3xl font-bold text-stone-800 group-hover:text-primary transition-colors'>{dashData.doctors}</p>
              <p className='text-stone-500 font-medium'>Total Doctors</p>
            </div>
          </div>
        </div>
        ...
        <div className='bg-gradient-to-br from-teal-500/10 to-teal-600/10 border border-teal-500/20 p-6 rounded-2xl hover:border-teal-400/40 hover:shadow-xl hover:shadow-teal-500/20 transition-all duration-300 group'>
          <div className='flex items-center gap-4'>
            ...
            <div>
              <p className='text-3xl font-bold text-stone-800 group-hover:text-teal-600 transition-colors'>{dashData.appointments}</p>
      ...
      <div className='bg-white border border-stone-200 rounded-2xl shadow-lg overflow-hidden'>
        <div className='flex items-center gap-3 px-6 py-5 border-b border-stone-200 bg-stone-50'>
          ...
          <p className='text-stone-800 font-semibold text-lg'>Latest Bookings</p>
        </div>
        ...
        <div className='max-h-96 overflow-y-auto'>
          {
            dashData.latestAppointments.map((item,index)=>(
              <div className='flex items-center px-6 py-4 gap-4 hover:bg-stone-50 border-b border-stone-100 last:border-b-0 transition-all duration-200' key={index}>
                <img className='rounded-full w-12 h-12 border-2 border-stone-200' src={item.docData.image} alt="" />
                <div className='flex-1'>
                  <p className='text-stone-800 font-semibold text-base'>{item.docData.name}</p>
                ...
                <div className='text-right'>
                  {
                    item.cancelled ? (
                      ...
                      <button
                        ...
                        className='px-4 py-2 rounded-lg text-sm font-semibold bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white shadow-md hover:shadow-lg transition-all duration-200'
                      >
                        Cancel
                      </button>
                    )
