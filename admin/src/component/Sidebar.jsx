...
const Sidebar = () => {
  ...
  const {dtoken, unreadChatCount}=useContext(DoctorContext)
  return (
    <div className='min-h-screen bg-stone-50 border-r border-stone-200 flex flex-col'>
      ...
      }
      {
        dtoken && <ul className='text-stone-600 mt-5 space-y-1'>
          ...
          <NavLink className={({isActive})=>`flex items-center justify-between px-4 py-3.5 md:px-8 md:min-w-72 cursor-pointer transition-colors ${isActive? 'bg-primary/10 border-l-4 border-primary text-primary font-medium' : 'hover:bg-stone-100 hover:text-stone-800'}`} to={'/doctor-chats'}>
            <p className='hidden md:block'>Chats</p>
            {unreadChatCount > 0 && (
              <span className='ml-2 inline-flex items-center justify-center px-2 py-1 text-[11px] font-semibold rounded-full bg-red-100 text-red-700'>
                {unreadChatCount}
              </span>
            )}
          </NavLink>
          <NavLink className={({isActive})=>`flex items-center px-4 py-3.5 md:px-8 md:min-w-72 cursor-pointer transition-colors ${isActive? 'bg-primary/10 border-l-4 border-primary text-primary font-medium' : 'hover:bg-stone-100 hover:text-stone-800'}`} to={'/doctor-patients'}>
